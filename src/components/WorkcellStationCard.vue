<template>
  ...
  >
    <h2 class="text-xl font-bold mb-2 text-cyan-300 drop-shadow">
      {{ workcell.name }}
    </h2>

    <div v-for="(s, i) in workcell.stations" :key="i" class="mb-2">
      <!-- Station label -->
          ...
          <template v-if="hasAnomaly(s)">
            ...
            <template v-for="(an, ai) in s.anomalies.filter((a) => a.end === null)" :key="ai">
              ...
              <span
                ...
                :key="fi"
                class="px-2 py-[1px] rounded bg-yellow-500/20 text-yellow-300 border border-yellow-400/30"
              >
                {{ ANOMALY_MAP[flag] }}
              </span>
            </template>
...
function hasAnomaly(station) {
  return station.anomalies.some((a) => a.anomalies_flag?.length > 0 && a.end === null)
}
</script>
